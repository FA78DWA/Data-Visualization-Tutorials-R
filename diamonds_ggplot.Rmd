...
## We can divide it into smaller scatter plots in terms of "clarity" by adding the "facet_wrap()" layer, and specifiny the feature that we want to use to divide the plot, in this case "~clarityS". 
...
## We can divide the big histogram the represents the whole data into smaller ones with respect to some feature, using "facet_wrap(~ feature_name)". For example, we can divide the histogram above wrt "clarity"
```{r}
